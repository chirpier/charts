...
function resolveChirpierChartState(props) {
...
function toEmbedOptions(props) {
    const { aggregate, compare, errorFallback, header, loadingFallback, range, tracker, variant, view, ...embedProps } = props;
    const resolvedState = resolveChirpierChartState(props);
    return {
        ...embedProps,
        state: resolvedState,
        style: embedProps.style,
    };
}
...
function resolveResizeMode(props) {
    if (props.resizeMode)
...
function applyIframeSizing(iframe, props, resizeMode) {
    ...
    if (props.aspectRatio !== undefined)
        iframe.style.aspectRatio = String(props.aspectRatio);
    if (resizeMode === "fill" && !height)
        iframe.style.height = "100%";
}
export const ChirpierChart = forwardRef(function ChirpierChart(props, ref) {
    ...
    let renderedErrorFallback = null;
    if (renderError) {
    ...
    return (React.createElement("div", { style: wrapperStyle },
        ...
        showLoadingFallback ? (React.createElement("div", { style: {
                position: "absolute",
                inset: 0,
                display: "flex",
                alignItems: "stretch",
